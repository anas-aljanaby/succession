...
export const OrganizationForm: React.FC = () => {
  ...
  return (
    <section className="mx-auto max-w-[1180px]">
      <PageHeader
        title={existing ? t('form.editOrg') : t('form.createOrg')}
        subtitle={
          existing ? <span className="text-[var(--text)]">{existing.name}</span> : undefined
        }
      />
      ...
      <form onSubmit={onSubmit} className="space-y-6">
        ...
        <div className="surface-card p-5">
          <h2 className="mb-4 text-sm font-semibold text-[var(--text)]">{t('form.contact')}</h2>
